...
import { BarChart2, Globe, ShoppingBag, Tag, ShieldCheck } from 'lucide-react'
...
const MiniProductCard = ({ img, name, price, badge }) => (
  <div
    className="flex min-w-0 flex-col overflow-hidden rounded-xl border text-start"
  ...
  >
    <div className="relative overflow-hidden" style={{ aspectRatio: '3 / 4' }}>
      ...
      {badge && (
        <span
          className="absolute start-1 top-1 rounded-full px-1.5 py-0.5 text-[7px] font-bold uppercase tracking-wide"
          style={{
            background: css.p10,
            color:      css.primary,
...
const StorefrontMockup = () => {
  ...
  return (
    <div
      className="overflow-hidden rounded-xl border shadow-sm text-start"
      style={{ background: css.surface, borderColor: css.border }}
    >
      {/* Browser bar */}
      <div
      ...
      >
        <div>
          <div className="font-heading text-[10px] font-extrabold" style={{ color: css.fg }}>
            {t('whatYouGet.groups.storefront.mockup.shopName', 'Your Shop')}
          </div>
          <div className="text-[8px] mt-0.5" style={{ color: css.mutedFg }}>
            {t('whatYouGet.groups.storefront.mockup.shopSubtitle', '4 products · Summer Collection')}
          </div>
        </div>
        <span
        ...
        >
          {t('whatYouGet.groups.storefront.mockup.verifiedBadge', 'Verified ✓')}
        </span>
      </div>
      ...
      >
        <span className="text-[9px]" style={{ color: css.mutedFg }}>
          {t('whatYouGet.groups.storefront.mockup.footerText', '4 products · stallio.shop/yourshop')}
        </span>
        <span className="text-[9px] font-bold" style={{ color: css.primary }}>
          {t('whatYouGet.groups.storefront.mockup.shopNow', 'Shop now →')}
        </span>
      </div>
    </div>
  )
...
const OrdersMockup = () => {
  ...
  return (
    ...
    >
      ...
      >
        {t('whatYouGet.groups.orders.mockup.title', 'Orders — Today')}
      </div>
      {[
        { id: '#1042', name: 'Zayn M.',   amt: '₨ 2,800', status: t('whatYouGet.groups.orders.mockup.statusPaid', 'Paid'),    variant: 'success' },
        { id: '#1041', name: 'Sara K.',   amt: '₨ 1,200', status: t('whatYouGet.groups.orders.mockup.statusPending', 'Pending'), variant: 'info'    },
        { id: '#1040', name: 'Ali R.',    amt: '₨ 5,500', status: t('whatYouGet.groups.orders.mockup.statusPaid', 'Paid'),    variant: 'success' },
      ].map((order) => (
        <div
          key={order.id}
  ...
  return (
    ...
    >
      <div
        ...
        style={{ borderColor: css.border, color: css.mutedFg, background: css.surfaceMuted }}
      >
        {t('whatYouGet.groups.commerce.mockup.title', 'Checkout Summary')}
      </div>
      <div className="space-y-2 p-3">
        <div className="flex justify-between text-[10px]">
          <span style={{ color: css.mutedFg }}>{t('whatYouGet.groups.commerce.mockup.item', 'Sneakers × 1')}</span>
          <span className="font-semibold" style={{ color: css.fg }}>₨ 6,800</span>
        </div>
        <div className="flex justify-between text-[10px]">
          <span style={{ color: css.mutedFg }}>{t('whatYouGet.groups.commerce.mockup.delivery', 'Delivery')}</span>
          <span className="font-semibold" style={{ color: css.fg }}>₨ 200</span>
        </div>
        <div className="flex justify-between text-[10px]">
          <span style={{ color: css.success }}>{t('whatYouGet.groups.commerce.mockup.coupon', 'Coupon SAVE15')}</span>
        ...
        >
          <span style={{ color: css.fg }}>{t('whatYouGet.groups.commerce.mockup.total', 'Total')}</span>
  ...
  return (
    ...
    >
      <div
        ...
        style={{ borderColor: css.border, color: css.mutedFg, background: css.surfaceMuted }}
      >
        {t('whatYouGet.groups.growth.mockup.title', 'Invoice · #INV-1042')}
      </div>
      <div className="p-3">
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="text-[10px] font-bold" style={{ color: css.fg }}>
              {t('whatYouGet.groups.growth.mockup.invoiceHeading', 'Stallio Invoice')}
            </div>
            <div className="text-[9px] mt-0.5" style={{ color: css.mutedFg }}>
              {t('whatYouGet.groups.growth.mockup.invoiceSubtitle', 'Aug 2026 · Zayn M.')}
            </div>
          </div>
        ...
        <div className="flex items-center justify-between text-[9px] mb-3" style={{ color: css.mutedFg }}>
          <span>{t('whatYouGet.groups.growth.mockup.revenueWeek', 'Revenue / week')}</span>
        ...
        >
          ...
          {t('whatYouGet.groups.growth.mockup.shareWhatsApp', 'Share via WhatsApp')}
        </button>
      </div>
    </div>
  )
...
const GroupCard = ({ group, delay, reducedMotion }) => {
  ...
  return (
    ...
    >
      ...
      {group.accent && (
        <div
          className="pointer-events-none absolute -end-20 -top-20 h-56 w-56 rounded-full opacity-[0.15] blur-3xl"
          aria-hidden="true"
      ...
      {/* ── Card content ── */}
      <div className={`flex flex-col gap-4 p-5 sm:p-6 text-start${group.wide ? ' lg:max-w-xs' : ''}`}>
        {/* Icon */}
        <div
        ...
        <div className="flex flex-wrap gap-1.5">
          {Array.isArray(group.chips) && group.chips.map((chip, i) => (
            <FeatureChip key={i}>{chip}</FeatureChip>
          ))}
        </div>
      </div>
      ...
      {/* ── Mockup panel ── */}
      <div
        className={`border-t p-4 text-start${group.wide ? ' flex-1 lg:border-s lg:border-t-0 lg:p-4' : 'shrink-0'}`}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ color: css.mutedFg }}>{t('whatYouGet.headlineSecondary')}</span>
      </motion.h2>
  ...
  const groups = [
    {
      id:       'storefront',
      Icon:     Globe,
      label:    t('whatYouGet.groups.storefront.label', 'Your Store Link'),
      headline: t('whatYouGet.groups.storefront.headline', 'One link. Your entire brand.'),
      body:     t('whatYouGet.groups.storefront.body', 'Share stallio.shop/yourname from your bio, WhatsApp, or QR. We host it, no domain, no server.'),
      chips:    t('whatYouGet.groups.storefront.chips', { returnObjects: true }) || ['Custom slug', 'Arabic & English', 'About & Contact pages', 'Mobile-first'],
    ...
    },
    {
      id:       'orders',
      Icon:     ShoppingBag,
      label:    t('whatYouGet.groups.orders.label', 'Order Management'),
      headline: t('whatYouGet.groups.orders.headline', 'Every order, organized.'),
      body:     t('whatYouGet.groups.orders.body', 'Search, filter, mark paid, set delivery status. Nothing lost in DMs or scattered across chats.'),
      chips:    t('whatYouGet.groups.orders.chips', { returnObjects: true }) || ['Order inbox', 'Status tracking', 'Buyer messages', 'Export CSV'],
      Mockup:   OrdersMockup,
    },
    {
      id:       'commerce',
      Icon:     Tag,
      label:    t('whatYouGet.groups.commerce.label', 'Catalog & Checkout'),
      headline: t('whatYouGet.groups.commerce.headline', 'Products buyers can actually buy.'),
      body:     t('whatYouGet.groups.commerce.body', 'Variants, sale prices, coupons, delivery fees, and COD, all applied at checkout with no gateway.'),
      chips:    t('whatYouGet.groups.commerce.chips', { returnObjects: true }) || ['Unlimited products', 'Variant pricing', 'Coupon codes', 'COD toggle'],
      Mockup:   CommerceMockup,
    },
    {
      id:       'growth',
      Icon:     BarChart2,
      label:    t('whatYouGet.groups.growth.label', 'Invoices & Insights'),
      headline: t('whatYouGet.groups.growth.headline', 'Professional receipts. Real numbers.'),
      body:     t('whatYouGet.groups.growth.body', 'PDF invoices per order, revenue charts, and a WhatsApp share button, all in one dashboard tap.'),
      chips:    t('whatYouGet.groups.growth.chips', { returnObjects: true }) || ['PDF invoices', 'Revenue overview', 'WhatsApp share', 'Seller support'],
      Mockup:   GrowthMockup,
    },
  ]
