    ...
    >
      <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <div ref={ref} className="relative mt-14 pl-10 sm:pl-14">
          {/* Track */}
          <div
            className="absolute left-3 top-2 bottom-2 w-px sm:left-5"
            style={{ background: 'var(--border)' }}
            aria-hidden="true"
          />
          {/* Progress fill */}
          <motion.div
            className="absolute left-3 top-2 w-px sm:left-5"
            style={{
          ...
          <ol ref={inViewRef} className="space-y-9 text-left">
            {LIFECYCLE.map((stage, i) => (
              <motion.li
                ...
                transition={{ duration: 0.55, delay: 0.1 + i * 0.12, ease: easePremium }}
              >
                {/* Node */}
                <span
                  className="absolute -left-10 top-1 flex h-6 w-6 items-center justify-center rounded-full border text-[10px] font-bold tabular-nums sm:-left-[3.25rem]"
                  style={{
